...
export function FlamegraphMinimap({
  ...
  // Calculate viewport position when focused
  ...
  const handleClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Find the span at this position (prefer deeper spans)
    const clickedBars = bars
      .filter(bar => {
        return clickPercent >= bar.leftPercent &&
               clickPercent <= bar.leftPercent + bar.widthPercent
      })
  ...
  return (
    ...
    >
      ...
      {viewportStyle && (
        <div
          className="absolute inset-y-0 border-2 border-primary bg-primary/20 rounded-sm cursor-grab active:cursor-grabbing"
          style={viewportStyle}
        />
      )}
