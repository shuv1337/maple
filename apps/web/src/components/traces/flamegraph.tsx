...
function calculateBars(
  ...
  // Get the reference point for calculations
  ...
  if (focusedSpan) {
    // When focused, only render the focused span and its descendants
...
function assignLanes(bars: FlamegraphBar[]): { bars: FlamegraphBar[]; totalLanes: number } {
  ...
  for (const depth of depths) {
    ...
    // Sort by leftPercent so greedy first-fit works correctly
    group.sort((a, b) => a.leftPercent - b.leftPercent)

    // Each lane tracks its right edge (leftPercent + widthPercent)
    const lanes: number[] = []
...
export function Flamegraph({
  ...
  const handleBarClick = (span: SpanNode, e: React.MouseEvent) => {
    // Shift+click or right-click to zoom
    if (e.shiftKey) {
    ...
    <div className="rounded-md border">
      ...
      <div className="flex items-center justify-between border-b bg-muted/30 px-3 py-2">
        <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground min-w-0">
          <span className="shrink-0">Flamegraph</span>
          {focusedSpan && (
            <>
              <span className="text-foreground/40 shrink-0">/</span>
              <span className="font-mono text-foreground truncate min-w-0" title={focusedSpan.spanName}>{focusedSpan.spanName}</span>
              <span className="text-foreground/40 shrink-0">({formatDuration(focusedSpan.durationMs)})</span>
            </>
          )}
        </div>
        {focusedSpan && (
      ...
      >
        ...
        <div className="relative px-3 pt-2" style={{ height: totalLanes * (ROW_HEIGHT + ROW_GAP) + 8 }}>
          ...
          {allBars.map((bar) => {
            const barCacheInfo = getCacheInfo(bar.span.spanAttributes)
            const cacheBarColor = barCacheInfo?.result === "hit"
              ? { backgroundColor: "oklch(0.75 0.15 80)" }
              : barCacheInfo?.result === "miss"
                ? { backgroundColor: "oklch(0.65 0.15 230)" }
                : null
            const colorStyle = bar.span.statusCode === "Error"
              ? {}
              : cacheBarColor ?? getSpanColorStyle(bar.span.spanName, bar.span.serviceName, services)

            return (
              <Tooltip key={bar.span.spanId}>
                <TooltipTrigger
                  className={cn(
                    "absolute flex items-center overflow-hidden rounded-md px-2 text-left font-mono text-[11px] font-medium shadow-sm cursor-pointer transition-[filter,box-shadow] duration-100",
                    "hover:brightness-125 hover:shadow-md hover:z-10",
                    bar.span.statusCode === "Error" && "bg-destructive ring-1 ring-inset ring-red-500/50 text-destructive-foreground",
                    hoveredSpan?.spanId === bar.span.spanId && "brightness-125 shadow-md z-10",
      ...
      <div className="flex items-center justify-between border-t bg-muted/30 px-3 py-2 text-[11px] text-muted-foreground">
        <div className="flex items-center gap-3 text-foreground/30">
          <span>
            <kbd className="rounded border border-foreground/10 bg-muted px-1 py-0.5 font-mono text-[10px]">Click</kbd> select
          </span>
          <span>
            <kbd className="rounded border border-foreground/10 bg-muted px-1 py-0.5 font-mono text-[10px]">Shift+Click</kbd> zoom
          </span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            <div className="h-2.5 w-2.5 rounded-sm bg-destructive shadow-sm" />
